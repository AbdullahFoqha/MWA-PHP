...
const get = (req, res) => {
    Anime.findById(req.params.animeId).select('arcs').exec((err, anime) => {
        if (err) {
            console.log(err)
        ...
        res.status(200).json(anime.arcs)
    })
}
...
const getById = (req, res) => {
    ...
    Anime.findById(animeId).select('arcs').exec((err, anime) => {
        if (err) {
            return res.status(500).send('something went wrong!, try again later')
        }
        ...
        if (!arc) {
            return res.status(404).send('Arc Not Found')
        }
...
const addArc = (req, res, anime) => {
    ...
    anime.save((err, newAnime) => {
        ...
        res.status(200).json(newAnime.arcs)
    })
}
